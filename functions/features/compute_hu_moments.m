function hu = compute_hu_moments(mask, image)
    % Se è presente l'immagine applica la maschera
    if nargin == 2 && ~isempty(image)
        % se l'immagine e' a colori converto in scala di grigi
        if size(image, 3) ~= 1 
          image = rgb2gray(image);
        end
        % Converto in double per evitare problemi di tipo e applica la maschera
        img = double(image) .* double(mask);
    else
        % Se l'immagine in scala di grigi non viene fornita, uso la maschera stessa
        img = double(mask);
    end
