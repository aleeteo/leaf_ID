function pred = classify_multiple(img, mask, classifier, recognizer, minmax)
%CLASSIFY_MULTIPLE Classifica gli oggetti segmentati in un'immagine
  ...
  for i = 1:num_labels
    item_mask = comps == i;
    label = classify_object(img, item_mask, classifier, recognizer, class_names, rec_names, minmax);
    pred(item_mask) = label;
  end
end
function label = classify_object(img, item_mask, classifier, recognizer, class_names, rec_names, minmax)
